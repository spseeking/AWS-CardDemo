...
CICS checked transaction security before the program ran, so the endpoints do the same.
...
carrying a clear password from a mainframe extract is migrated to bcrypt on the next successful
sign on and the clear field is blanked, so a legacy password works exactly once.
